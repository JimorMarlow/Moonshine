...
def read_version():
    with open('src/version.h', 'r', encoding='utf-8') as f:
        text = f.read()
        major = re.search(r'#define\s+MS_VERSION_MAJOR\s+(\d+)', text).group(1)
        minor = re.search(r'#define\s+MS_VERSION_MINOR\s+(\d+)', text).group(1)
        patch = re.search(r'#define\s+MS_VERSION_PATCH\s+(\d+)', text).group(1)
...
print(f"<Moonshine> version updated to {version} in metadata files")
#exit(0)
