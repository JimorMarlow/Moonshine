...
namespace moonshine 
{
    ...
    struct state_t
    {
        ...
        etl::optional<float> deflegmater_water_flow = 0.0; // Скорость потока воды ml/min на выходе дефлегматора
        etl::optional<float> condenser_water_flow = 0.0;   // Скорость потока воды ml/min на выходе охладителя
        ...
        void trace() {
            ...
            Serial.printf("deflegmater_water_flow = %s\n", deflegmater_water_flow ? String(*deflegmater_water_flow).c_str() : "---");
            Serial.printf("condenser_water_flow = %s\n", condenser_water_flow ? String(*condenser_water_flow).c_str() : "---");
            Serial.println("========================");
        }            
    };
