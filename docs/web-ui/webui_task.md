...
#### Этап 2: Исправление mDNS (v1.2.3) ✅
**Дата:** 2026-02-25

**Проблема:**
- Устройство не отвечало по mDNS (http://moonshine.local)
- Работало только по прямому IP-адресу
- Проблема проявлялась в обоих режимах (AP и STA)

**Причина:**
- Отсутствовал вызов `MDNS.update()` в цикле обработки
- На ESP8266 это критично для поддержания работы mDNS службы

**Исправления:**
- Добавлен `MDNS.update()` в `handleClient()` перед `m_server.handleClient()`
- Изменён порядок инициализации WiFi: `WiFi.hostname()` вызывается до `WiFi.mode()`

**Обновления в документации:**
- WEB_UI-HISTORY.md: добавлена запись v044 🔧 Исправление mDNS (без изменений макета)
- version.h: обновлена версия до 1.2.3
